...
namespace SimpleDIContainerTest
{
    public static class CatExtensions
    {
        ...
        {
            cat.Register(new ServiceRegistry(serviceType, lifetime, (_, arguements) => factory(_)));
            return cat;
        }

        public static Cat Register<TService>(this Cat cat, Func<Cat, TService> factory, Lifetime lifetime)
        {
            cat.Register(new ServiceRegistry(typeof(TService), lifetime, (_, arguements) => factory(_)));
            return cat;
        }
        ...
        private static object Create(Cat cat, Type type, Type[] genericArguments)
        {
            ...
            {
                arguments[i] = cat.GetService(parameters[i].ParameterType);//debug看看走的哪条代码
            }
